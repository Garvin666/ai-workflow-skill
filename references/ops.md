...
> **变更日志已拆到 `_archive/changelog.md`**（v3.4.0 / P4）—— 它是**追加式无上限增长**的台账，
...
| **laya_client.py**（v4.7.0 自研） | **Laya 离线决策服务客户端**（Realization B 的本地落地；**影子期只作对照后端，不接管路由**）：`--judge self-judge\|method-judge\|retrieval-judge\|homework-judge` 把 judge 入参填进 `judges.json` 模板 → `POST /v1/judge` 到本地服务 → 按契约映射输出 `{category, distribution, confidence, dimensions, ambiguity, route_hint, _laya}`（`homework-judge` 输出 `mode` + `needs_homework`，**不产 `secondary`**）；`--selfcheck` 探测服务可用性。**纯标准库**（零新增依赖）。超时/重试沿用 `RETRY_DELAYS=(0,2,4,8)`，调判定前先 3 s 探 `/readyz` **快速降级**；服务不可达抛 `LayaUnavailable` → **exit 3 + `{"degraded": true}`**，调用侧回退 Realization A、**流程不中断**。⚠️ 契约的 `confidence` 取 `max(distribution)`，Laya 自带的**熵置信**另存 `_laya.entropy_confidence` —— 两量不同源，已在单测断言，勿混用。⚠️ **本地回环请求会绕过环境代理**（`_opener()`：回环走 `ProxyHandler({})`）—— 否则本机 `HTTP_PROXY` 会把 `127.0.0.1` 请求答成 502，导致服务在跑却被判不可用、静默降级（v4.7.0 修，见 `laya-backend.md` §4.1）。⚠️ `PINNED_MODEL_BY_KIND` 对 `method_judge`/`retrieval_judge`/`homework_judge` 钉 `multilingual`（`--model` 显式入参优先）；`homework_judge` 一项**未经真机复核**。环境变量：`LAYA_BASE_URL`／`LAYA_PORT`（默认 8731）／`LAYA_JUDGE_TOKEN`（**只经环境变量，不落盘**）／`LAYA_CONNECT_TIMEOUT`／`LAYA_READ_TIMEOUT`。部署事实、鉴权门禁与实测边界见 `references/laya-backend.md` | `"$PY" scripts/laya_client.py --judge self-judge --state "帮我写个脚本合并这三份 CSV"`<br>`"$PY" scripts/laya_client.py --judge homework-judge --state "求 f(x)=x^2-4x+3 的最小值"`<br>`"$PY" scripts/laya_client.py --selfcheck` |
| **homework_model.py**（v4.7.0 自研） | **模式选用聚合模型**（`homework-judge` 的 **Realization A 可执行半边**）：W1–W5 维度分 → 三态分布 / `mode`（含 `A+B` 组合）/ `confidence` / 档位（`auto`\|`mid`\|`clarify`）/ 派生 `needs_homework`；公式即 `references/homework-judge.md` §5.1（**唯一口径源**），锚点 `ANCHORS` 与温度 `TAU` 的数值**只写在脚本一处**（手册与 `judges.json` 不复制，从根上消除双写漂移面）。`--plan-block` 直接产出可粘进 `plan.yaml` 的 `meta.作业判定` 片段（YAML 标量走 JSON 双引号形式，避开手写转义坑）；`agree --pairs <json>` 算 A/B 一致率（`mode_exact` / `mode_set` / `needs_homework` / `dist_l1` + 分歧清单），补上契约 §7.3 出口条件的**测量环节**。**纯标准库**。⚠️ 它**不做识别**（不从文本猜维度）—— 语义归模型/人、算术归代码；`ANCHORS`/`TAU` 是**未拟合先验**（`_model.anchors_fitted` 恒 `false`），`distribution`/`confidence` 只可用于对照与回归，**不得**据此声称准确率或定阈值；`agree` **刻意不给默认阈值**（未传 `--min-agree` 时只报告不判定）；⚠️ 脚本**不产生样本** —— 没有真机 B 侧输出时仍无数据可测 | `"$PY" scripts/homework_model.py aggregate --dims "W1=0.9,W2=0.85,W3=0.6,W4=0.2,W5=0.85"`<br>`"$PY" scripts/homework_model.py agree --pairs <pairs.json>`<br>`"$PY" scripts/homework_model.py selftest` |
| **checks_core.py / checks_parity.py / checks_judges.py / checks_cmds.py**（v4.4.2 拆分） | `checks.py` 的**内部模块**（**不由用户直接调用**，勿单独运行）：`core`＝import + 模块级常量 + 通用工具（不依赖其余模块）；`parity`＝枚举解析与跨文件同源校验（含 `PARITY_ITEMS`）；`judges`＝**20 个** `_check_*` 判据（v4.7.0 实测；计数口径＝模块级 `def _check_*`，v4.4.2 时 16 个）；`cmds`＝7 个子命令实现。**常量计数口径**：模块级大写赋值（v4.7.0 实测 `core` 为 **64** 个 `Assign` + 1 个 `AnnAssign`）—— ⚠️ 该口径由本次顺手实测给出，与文档早前写的"56 个"**不同口径**，**不可直接相减/比对**。依赖方向 **core ← parity ← judges ← cmds ← `checks.py`（入口）**，严格单向无循环。`checks.py` 保留为入口并 **re-export 全部常量** —— `gate.py`（按文件路径加载该模块、读**模块属性** `INFRA_EXCEPTIONS`）依赖它，**改动时不得破坏**。⚠️ 但 `guard_constants.py` **不**走这条路径：它按 **AST** 取模块级 `Assign` 节点，而星号导入不产生 `Assign` ⇒ 其 `GROUPS.members` **必须登记常量的定义处**（`checks_core.py`），登记入口 `checks.py` 会恒报 MISSING、守卫对那几组**静默失能**（2026-09-22 修复，见 CHANGELOG v4.6.0） | `python scripts/checks.py skill`（入口用法与拆分前完全一致） |
