...
- **v3.1.0（2026-09-15）新增「自研工具与技能：标注与公开留痕」（贯穿阶段 3–6）**（用户 2026-09-15 指定；方案评审三选：新建独立仓 `Garvin666/ai-workflow-tools` ／ 文档规则 + 机器门禁 ／ **不追溯存量**）：
  ...
  - **诚实边界（未解决，如实登记）**：① 门禁只能校验"登记项是否完整"，**不能自动发现「代码里写了自研工具却没登记」** —— 漏登记仍靠阶段 5 自检与人工；② `verify` 只比对**显式传入的文件**，不扫全仓；③ 目标仓 `Garvin666/ai-workflow-tools` **截至本条写入时尚未创建**（创建 public 仓属不可逆对外动作，须用户单独确认），故本任务 plan 的 `自研工具.仓库链接` 现为『待推送』，创建后回填；④ 按用户决定**不追溯存量** —— 本机既有自研脚本不补登记、不补推送。
